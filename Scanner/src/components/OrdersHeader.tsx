...
/** Mock data for companies and dentists — Figma: QACompanyQA, Dr. Rudnik */
const COMPANIES = [
...
export const SHOW_ALL_DRS_ID = "show-all-drs" as const;
...
export default function OrdersHeader({
  ...
  const [internalDentistId, setInternalDentistId] = useState(DENTISTS[0].id);
  const isControlled = controlledDentistId !== undefined && onDentistChange !== undefined;
  const selectedDentistId = isControlled ? controlledDentistId : internalDentistId;
  const setSelectedDentistId = isControlled ? onDentistChange : setInternalDentistId;
  ...
  const dentistDisplayLabel =
    selectedDentistId === SHOW_ALL_DRS_ID ? "Show all Drs" : `${selectedDentist.name} - ${selectedDentist.role}`;
  ...
  return (
    ...
    >
      {/* Left: Home icon + separator + title */}
